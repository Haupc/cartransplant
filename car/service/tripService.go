...
type TripService interface {
	...
	FindPendingTrip(from, to, date, seat int32, tripType []int32) (*grpcproto.ListUserTripResponse, error)
...
func (s *tripService) UpdateUserTrip(driverTripID, userTripID, userTripPrice int32) error {
	passengerTrip, err := s.PassengerTripRepo.FindPassengerTripByID(userTripID)
	if err != nil {
		log.Printf("UpdateUserTrip - Error: %v", err)
		return err
	}
	passengerTrip.TripID = int64(driverTripID)
	passengerTrip.Price = int64(userTripPrice)
	passengerTrip.State = 2
...
func (s *tripService) FindPendingTrip(from, to, date, seat int32, tripType []int32) (*grpcproto.ListUserTripResponse, error) {
...
func (s *tripService) ListDriverTrip(userID string, state, startDate, endDate int32) (*grpcproto.ListDriverTripResponse, error) {
	...
	for _, tripModel := range tripModels {
		...
		driverTrip := &grpcproto.DriverTrip{
			...
			EndLeaveTime:   tripModel.EndLeaveTime,
...
func (s *tripService) FindTrip(from *grpcproto.Point, to *grpcproto.Point, beginLeaveTime int64, endLeaveTime int64, tripType, seat int32) ([]trip_dto.FindTripResponse, error) {
	...
	for _, m := range models {
		...
		realBeginLeaveTime := float64(m.BeginLeaveTime) + subRoute.Routes[0].Duration
		realEndLeaveTime := float64(m.EndLeaveTime) + subRoute.Routes[0].Duration
...
func (s *tripService) TakeTrip(userID string, driverTripID, beginLeaveTime, endLeaveTime int64, seat int32, from, to *grpcproto.Point, note string) error {
	...
	GetNotifyService().NotifyNewUserRegisterTrip(driverTrip.UserID, int32(driverTripID))
	return err
}
...
func (s *tripService) ListUserTrip(userID string, state int32) (*grpcproto.ListUserTripResponse, error) {
	...
	for _, u := range userTrips {
		var dirverInfo *grpcproto.UserProfile
