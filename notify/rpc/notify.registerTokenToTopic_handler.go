...
func (n *notifyServer) RegisterTokenToTopic(ctx context.Context, req *grpcproto.RegisterTokenToTopicRequest) (*grpcproto.Bool, error) {
	log.Printf("RegisterTokenToTopic\n")
	tokens := n.userTokenRepo.GetAllTokenByUserID(req.UserID)
	result, err := n.fcmClient.SubscribeToTopic(ctx, tokens, req.Topic)
	if err != nil {
